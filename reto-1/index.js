...
textSplited.forEach((word) => {
  ...
  if (objectGroup[wordLower]) {
    objectGroup[wordLower] = objectGroup[wordLower] + 1;
  } else {
    objectGroup[wordLower] = 1;
  }
});

for (var prop in objectGroup) {
  result += prop + objectGroup[prop];
}
